        ...
        protected void JoinChannel()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(ListUtils.MySettingsList?.AgoraAppCertificate))
                {
                    ...
                    token = tokenBuilder.BuildTokenWithUid(AppSettings.AppIdAgoraLive, ListUtils.MySettingsList?.AgoraAppCertificate, channelName, uid, RtcTokenBuilder.Role.RolePublisher, timestamp);
                }
