    ...
    public class IntentController
    {
        ...
        public void OpenFacebookIntent(string name)
        {
            try
            {
                ...
                Intent facebookIntent = new Intent(Intent.ActionView, Uri.Parse("fb://facewebmodal/f?href=https://www.facebook.com/" + name));  //Try's to make intent with FB is URI

                if (facebookIntent.ResolveActivity(Context.PackageManager) != null)
                    Context.StartActivity(facebookIntent);
                else
                    Context.StartActivity(new Intent(Intent.ActionView, Uri.Parse("https://www.facebook.com/" + name)));  //catches and opens a url to the desired page
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
